...
bool MainObject::LoadImg(std::string path, SDL_Renderer* screen)
{
  ...
  if (ret == true)
  {
    width_frame_ = rect_.w/NUM_FRAME; // ?? dài c?a 1 frame
...
void MainObject::set_clips() // Ch?y frame
{
  //Clip the sprites
...
void MainObject::DoPlayer(Map& g_map) // Xử lí di chuyển nhân vật
{
  if (think_time_ == 0)
  {
    x_val_ = 0;
...
void MainObject::CheckToMap(Map& g_map) // Va chạm với tile map
{
  int x1 = 0;
  int x2 = 0;
  int y1 = 0;
